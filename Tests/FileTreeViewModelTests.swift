...
final class FileTreeViewModelTests: XCTestCase {
    ...
    @MainActor
    func testPreAddTreeSortsNamesBothDirections() {
        let pending = PendingTorrent(
            source: .file(path: "/tmp/test.torrent"),
            name: "Test",
            totalSize: 30,
            savePath: "/tmp",
            files: [
                PendingFile(id: 0, path: "zeta.mkv", size: 10),
                PendingFile(id: 1, path: "Alpha.mkv", size: 20),
            ]
        )
        let model = PreAddViewModel(pending: pending)

        model.sort(by: .nameAsc)
        XCTAssertEqual(model.tree.map(\.name), ["Alpha.mkv", "zeta.mkv"])

        model.sort(by: .nameDesc)
        XCTAssertEqual(model.tree.map(\.name), ["zeta.mkv", "Alpha.mkv"])
    }

    @MainActor
    func testPreAddTreeSortsBySize() {
        let pending = PendingTorrent(
            source: .file(path: "/tmp/test.torrent"),
            name: "Test",
            totalSize: 30,
            savePath: "/tmp",
            files: [
                PendingFile(id: 0, path: "small.mkv", size: 10),
                PendingFile(id: 1, path: "large.mkv", size: 20),
            ]
        )
        let model = PreAddViewModel(pending: pending)

        model.sort(by: .sizeDesc)
        XCTAssertEqual(model.tree.map(\.name), ["large.mkv", "small.mkv"])
    }
}
